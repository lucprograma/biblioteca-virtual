...
if (modalRef.current) {
  ...
  const fetchYears = async (parent_id) => {
    try {
      const response = await fetch(`http://localhost:3000/api/folders/byParent/${parent_id}`);
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await fetch('http://localhost:3000/api/folders/parentsFolders');
        const data = await response.json();
        setCourses(data);
      } catch (error) {
